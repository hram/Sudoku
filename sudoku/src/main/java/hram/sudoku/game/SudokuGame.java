...
import hram.sudoku.game.command.AbstractCommand;
import hram.sudoku.game.command.ClearAllNotesCommand;
import hram.sudoku.game.command.CommandStack;
import hram.sudoku.game.command.EditCellNoteCommand;
import hram.sudoku.game.command.FillInNotesCommand;
import hram.sudoku.game.command.SetCellValueCommand;
...
public class SudokuGame {
    ...
    private int bitmapRawWidth, bitmapRawHeight;
    ...
    public void saveState(Bundle outState) {
        ...
        outState.putInt("bitmapRawWidth", bitmapRawWidth);
        outState.putInt("bitmapRawHeight", bitmapRawHeight);
        outState.putByteArray("bitmap", bitmap);
        outState.putParcelable("res", res);
    ...
    public void restoreState(Bundle inState) {
        ...
        framingRect = inState.getParcelable("framingRect");
        bitmapRawWidth = inState.getInt("bitmapRawWidth");
        bitmapRawHeight = inState.getInt("bitmapRawHeight");
        bitmap = inState.getByteArray("bitmap");
        res = inState.getParcelable("res");
        orientation = inState.getInt("orientation");
    ...
    public void setBitmapRaw(byte[] bm) {
        bitmap = bm;
    }

    public byte[] getBitmapRaw() {
        return bitmap;
    }

    public int getBitmapRawWidth() {
        return bitmapRawWidth;
    }

    public void setBitmapRawWidth(int width) {
        bitmapRawWidth = width;
    }

    public int getBitmapRawHeight() {
        return bitmapRawHeight;
    }

    public void setBitmapRawHeight(int height) {
        bitmapRawHeight = height;
    }

    public void setFramingRect(Rect res) {
        this.framingRect = res;
    }
    ...
    public Bitmap getCropedBitmap(Context context) {
        if (cropedBitmap == null) {
            ...
            long sudokuGameID = gameSettings.getLong(Constants.LAST_CROPED_SUDOKU_ID, -1);
            if (sudokuGameID == mId) {
                FileInputStream fis;
                try {
                    fis = context.openFileInput(Constants.LAST_CROPED_SUDOKU_NAME);
                    cropedBitmap = BitmapFactory.decodeStream(fis);
                    fis.close();
                } catch (Exception e) {
                }
            }
    ...
    public void setCropedBitmap(Bitmap bm) {
        cropedBitmap = bm;
    }
