...
import hram.sudoku.game.FolderInfo;
import hram.sudoku.game.SudokuGame;
import hram.sudoku.result.Result;
import hram.sudoku.utils.SudokuListFilter;
...
public class SudokuDatabase {
    public static final String DATABASE_NAME = "opensudoku.db";
    ...
    public SudokuGame getSudoku(long sudokuID) {
        ...
        try {
            ...
            if (c.moveToFirst()) {
                ...
                byte[] bitmap = c.getBlob(c.getColumnIndex(SudokuColumns.BITMAP));
                if (bitmap != null) {
                    s.setBitmapRaw(bitmap);
                }

                s.setBitmapRawHeight(c.getInt(c.getColumnIndex(SudokuColumns.BM_HEIGHT)));
                s.setBitmapRawWidth(c.getInt(c.getColumnIndex(SudokuColumns.BM_WIDTH)));

                Rect framingRect = new Rect(c.getInt(c.getColumnIndex(SudokuColumns.FR_LEFT)), c.getInt(c.getColumnIndex(SudokuColumns.FR_TOP)), c.getInt(c.getColumnIndex(SudokuColumns.FR_RIGHT)), c.getInt(c.getColumnIndex(SudokuColumns.FR_BOTTOM)));

                s.setFramingRect(framingRect);

                Point tl = new Point(c.getInt(c.getColumnIndex(SudokuColumns.RES_TL_X)), c.getInt(c.getColumnIndex(SudokuColumns.RES_TL_Y)));
                Point tr = new Point(c.getInt(c.getColumnIndex(SudokuColumns.RES_TR_X)), c.getInt(c.getColumnIndex(SudokuColumns.RES_TR_Y)));
                Point bl = new Point(c.getInt(c.getColumnIndex(SudokuColumns.RES_BL_X)), c.getInt(c.getColumnIndex(SudokuColumns.RES_BL_Y)));
                Point br = new Point(c.getInt(c.getColumnIndex(SudokuColumns.RES_BR_X)), c.getInt(c.getColumnIndex(SudokuColumns.RES_BR_Y)));
                Result res = new Result(tl, tr, bl, br, 0);

                s.setResult(res);

                s.setOrientation(c.getInt(c.getColumnIndex(SudokuColumns.ORIENTATION)));
            }
        } finally {
            if (c != null) c.close();
        }
     ...
     * @return
     */
    public long insertSudoku(long folderID, SudokuGame sudoku, byte[] image, int width, int height, Rect framingRect, Result res, int orientation) {
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        ...
        values.put(SudokuColumns.BITMAP, image);
        values.put(SudokuColumns.BM_HEIGHT, height);
        values.put(SudokuColumns.BM_WIDTH, width);
        values.put(SudokuColumns.FR_LEFT, framingRect.left);
        values.put(SudokuColumns.FR_TOP, framingRect.top);
        values.put(SudokuColumns.FR_RIGHT, framingRect.right);
        values.put(SudokuColumns.FR_BOTTOM, framingRect.bottom);
        values.put(SudokuColumns.RES_TL_X, res.TL.x);
        values.put(SudokuColumns.RES_TL_Y, res.TL.y);
        values.put(SudokuColumns.RES_TR_X, res.TR.x);
        values.put(SudokuColumns.RES_TR_Y, res.TR.y);
        values.put(SudokuColumns.RES_BL_X, res.BL.x);
        values.put(SudokuColumns.RES_BL_Y, res.BL.y);
        values.put(SudokuColumns.RES_BR_X, res.BR.x);
        values.put(SudokuColumns.RES_BR_Y, res.BR.y);
        values.put(SudokuColumns.ORIENTATION, orientation);
